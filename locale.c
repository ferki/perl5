...
#  else
    ...
    if (strEQ(PL_curlocales[NOMINAL_LC_ALL_INDEX], PL_curlocales[0])) {
        return mortalized_pv_copy(PL_curlocales[0]);
    }

    Size_t longest_index;

    /* If we haven't calculated the syntax, do so */
    if (UNLIKELY(PL_LC_ALL_separator_string_len == 0)) {

        /* First find the longest name that isn't "C".  We know such a one
         * exists, because if they were all "C" we would have returned before
         * getting here */
        for (PERL_UINT_FAST8_T i = 0; i < LC_ALL_INDEX_; i++) {
            if strEQ(PL_curlocales[i], "C") {
                continue;
            }

            longest_index = i;
            Size_t longest_length = strlen(PL_curlocales[i]);

            for (PERL_UINT_FAST8_T j = i + 1; j < LC_ALL_INDEX_; j++) {
                Size_t this_length = strlen(PL_curlocales[j]);
        ...
        /* We use the longest locale name to figure out the syntax.  By using
         * the longest, we don't have to worry about it being a substring of
         * something else.  (This is true even if all the names are all the
         * same length.) */
        const char * alternate = PL_curlocales[longest_index];
        ...
        /* Initialize */
        /* XXX May have to lock around this whole thing in case two threads
         * want to be setting up this global at the same time */
        for (unsigned int j = 0; j < LC_ALL_INDEX_; j++) {
            PL_map_LC_ALL_position_to_index[j] = -1;
        }
        ...
        for (unsigned int j = 0; j < NOMINAL_LC_ALL_INDEX; j++) {
            ...
            /* Then set this category to the longest-named locale */
            ...
            /* Then find what the system says LC_ALL looks like with just this
             * one category not set to 'C' */
            ...
            /* Here isn't name=value pairs.  Find the position of the
             * longest-named locale */
            const char * alt_pos = instr(lc_all, alternate);
            if (! alt_pos) {
                DEBUG_Lv(PerlIO_printf(Perl_debug_log,
            ...
            while (s < alt_pos) {
                 ...
                 * previous 'C'.  The separator starts here, ending just before
                 * the non-C locale
                 *
                ...
                /* Otherwise make sure it's the same string as previously
                 * calculated */
                ...
                /* Here, we have found the position of category[j] in LC_ALL.
                 * */ 
                break;
            }

            if (PL_map_LC_ALL_position_to_index[count] != -1) {
            ...
            /* Save the index of this category in its corresponding position */
            PL_map_LC_ALL_position_to_index[count] = j;
        } /* End of loop through all the positions */

#    ifdef DEBUGGING
        /* XXX when have an LC_foo but no USE_LC_foo */
        for (unsigned int j = 0; j < NOMINAL_LC_ALL_INDEX; j++) {
            const unsigned index = PL_map_LC_ALL_position_to_index[j];
...
#    endif
    ...
    } /* End of calculating the syntax of disparate LC_ALL */

    /* Here, the syntax has been calculated */
    ...
    for (PERL_UINT_FAST8_T i = 0; i < LC_ALL_INDEX_; i++) {
         ...
         * individual locales.  We don't care about the ordering, as that makes
         * no difference to the total */
        ...
        /* Then fill it in */
        for (unsigned pos = 0; i< NOMINAL_LC_ALL_INDEX; i++) {
            unsigned i = PL_map_LC_ALL_position_to_index[pos];
